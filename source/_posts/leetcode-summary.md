...
## 9.20 LeetCode 4035 最多有效分割位置I

这道题20260829首刷，是第190场双周赛的赛题。比赛时解出来了，用暴力法。这道题还有更好的优化但是我们在这里不赘述，我们说一下暴力法的时间复杂度计算问题。

这道题我们最麻烦的时间复杂度计算，是在像求前缀和一样求出前缀/后缀的最大公约数。我们以前缀最大公约数为例，实际上，我们每次遍历求前缀最大公约数的时间复杂度应该为$O(n + \log U)$，而不是想当然的$O(n\log U)$（$\log U$的时间复杂度来自于欧几里得算法）。为什么是加法而不是乘法呢，实际上，求前缀最大公约数，得到的答案一定是越来越小的。我们可以想象一下，假设数组中第0个和第1个元素的最大公约数是$a$，那么加入第二个元素之后，要么最大公约数跟之前一样，要么不能超过$a/2$。也就是说，这样一趟遍历的过程中，花费一次$\log U$的时间就能把这个答案给求出来，所以也就不是乘法，而是加法。

根据上面的推导，我们不难算出整体的时间复杂度为$O(n(n + \log U))$，在数据比较小的时候，也就不会超时了。
...
### 12.3.3 LeetCode 2370 最长理想子序列

本题20260807首刷。难点还是在状态的定义上。实际上，第一维的定义应该很清晰，第二维的定义，一般就是字符串问题的套路，要想到去枚举字符。个人觉得还是比较抽象，未来只有通过多练习的办法才能掌握这些问题。

![alt text](../img/LeetCode/image-163.png)
...
### 12.9.4 LeetCode 3180 执行操作可获得的最大总奖励I

这道题20260730首刷，是一道0-1背包问题的变种。这道题给我的一个启示就是，我们不能在做DP的时候去猜一个DP状态，而应该要搞清楚中间转移了一个什么信息，针对DP的题，还需要大量的练习。

此外，下面图中还有一个非常重要的点，就是枚举顺序一定不能搞错，如果从小到大遍历，可能在一次访问中会把$x < k - x < 2x - 1$的元素进行多次覆盖，导致结果出错。

![alt text](../img/LeetCode/image-154.png)

这个题比较难，首刷理解的也不是很深刻，值得二刷。
...
### 12.21.1 LeetCode 486 预测赢家

这道题20260801首刷，应该是首次见到博弈DP的类型，确实不会做。这道题一开始做的时候掉入了一个误区，就是不是说两个选手可以随便选，只要某一种情况成立了就行，而是说他们每一次做选择时，玩家不是瞎选、赌运气、看当下利益，而是会把「选左边、选右边」两条路未来所有走向全部推演一遍，预判对手后续所有操作，选出最终自己收益最大的那条路。所以这么一来，原来的递归就不是一个好的解法了。实际上，他就是二者的博弈，所以我们把它称作博弈DP。

我们来看一下灵神对这个问题的抽象：

![alt text](../img/LeetCode/image-155.png)

![alt text](../img/LeetCode/image-156.png)

好，接下来就是这个题目难的地方，就是原问题和子问题的关系到底是什么？如果数学思维跟不上，那就很难想到。
